...
EXCEL = 'VFX3_ADUS_Report.xlsx'

###########################################################################
def vfx3_adus_report():
    ...
    session.findById("wnd[0]/usr/ctxtVKORG").text = "0001"
...
def load_to_pandas():
    VFX3_ADUS_DF = pd.read_csv(FILEPATH + '/' + FILENAME, skiprows=3, sep='|', engine='python')
    
    cols = [c for c in VFX3_ADUS_DF.columns if c.lower()[:7] != 'unnamed'] # drops the empty unnamed columns
    VFX3_ADUS_DF = VFX3_ADUS_DF[cols]
    VFX3_ADUS_DF.drop([0, 0], inplace=True) # Drop first empty rows
    
    # VFX3_ADUS_DF.drop([0, 1], inplace=True) # Drop first empty rows
    # VFX3_ADUS_DF = VFX3_ADUS_DF[:-1] # Drop last row containing dash's --
    # VFX3_ADUS_DF.dropna(axis=1, how='all', inplace=True) # Drop Nan columns



    VFX3_ADUS_DF.to_excel(DIR + EXCEL, index=False)
    print(VFX3_ADUS_DF.head())
